...
def writeToTensorBoard(writer, tensorboardData, curr_episode):
    ...
    reward, value, policyLoss, qValueLoss, entropy, policyGradNorm, qValueGradNorm, log_alpha, alphaLoss, ceLoss = tensorboardData
...
def run_test(curr_episode):
    ...
    worker = TestWorker(meta_agent_id=None, policy_net=global_network, global_step=curr_episode % 152, test=False, device=device, save_image=save_img, greedy=False)
    worker.run_episode_heuristic(ml=10, curr_episode=curr_episode)
    perf_metrics = worker.perf_metrics
    heu_u = perf_metrics['heuristic_u']

    print('train_heur_u: ', heu_u)

    return heu_u

def write_test(writer, train_heu_u_list, curr_episode):
...
def main():
    ...
    for i, meta_agent in enumerate(meta_agents):
        curr_episode += 1
        job_list.append(meta_agent.sample_job.remote(weights_set, curr_episode))
    ...
    try:
        ...
        while True:
            ...
            for job in done_jobs:
                job_results, info = job
                for i in range(len(experience_buffer)):
                    experience_buffer[i] += job_results[i]
            
            # launch new task
            curr_episode += 1
            job_list.append(meta_agents[info['id']].sample_job.remote(weights_set, curr_episode))
            ...
            if curr_episode % 1 == 0 and len(experience_buffer[0]) >= MINIMUM_BUFFER_SIZE:
                ...
                for j in range(8):
                    ...
                    action_batch = torch.tensor(action_list).unsqueeze(1).unsqueeze(1).to(device)
                    reference_policy_batch = torch.tensor(rp_list).unsqueeze(1).unsqueeze(1).to(device)
                ...
                data = [reward_batch.mean().item(), value_prime_batch.mean().item(), policy_loss.item(), q1_loss.item(),
                        entropy.mean().item(), policy_grad_norm.item(), q_grad_norm.item(), log_alpha.item(), alpha_loss.item(), beta_loss.item()]
            ...
            if curr_episode % 32 == 0 or newmaxreward >= maxreward:
                ...
                if curr_episode % 1000 == 0:
                    path_checkpoint = "./" + model_path + "/checkpoint_{}.pth".format(curr_episode)
                    torch.save(checkpoint, path_checkpoint)
