...
class Worker:
    ...
    def select_node(self, observations, edge_for_select_node):
        node_inputs, edge_inputs, current_index, node_padding_mask, edge_padding_mask, edge_mask = observations
        ...
        all_pairs = get_paired_combinations(self.env.robot_indexes)

        max_distances = []
        max_combs = []

        for pairs in all_pairs:
            dist1 = self.env.dist_data[pairs[0][0], pairs[0][1], self.env.target_index]
            dist2 = self.env.dist_data[pairs[1][0], pairs[1][1], self.env.target_index]
            dist3 = self.env.dist_data[pairs[2][0], pairs[2][1], self.env.target_index]
        ...
        next_node = self.env.get_opponent_position(min_comb, self.env.target_index)
        next_node_index = edge_for_select_node[self.env.target_index].index(next_node)
        rp = next_node_index
        rp = torch.tensor(rp).unsqueeze(0).unsqueeze(0)
    ...
    def run_episode(self, curr_episode):
        if self.env.input_type == 'map':
            ...
            for i in range(128):
                ...
                action_index_list = []
                rp_list = []
                observations, edge_for_select_node = self.get_observations()
                self.save_observations(observations)

                next_position, action_index, rp = self.select_node(observations, edge_for_select_node)
                action_index_list.append(action_index)
                rp_list.append(rp)

                self.env.step(next_position, N_ROBOTS+1)

                # target move
                for robot_index in range(N_ROBOTS):
                    self.env.step(None, robot_index, previous_index)
                ...
                final_all_observations, _ = self.get_observations()
            ...
            for i in range(128):
                ...
                next_index, action_index, rp = self.select_node(observations, edge_for_select_node)

                action_index_list.append(action_index)
                rp_list.append(rp)

                self.env.step(next_index, N_ROBOTS+1)
