...
class QNet(nn.Module):
    ...
    def output_q_values(self, enhanced_node_feature, edge_inputs, current_index, edge_padding_mask, node_padding_mask):
        ...
        tensor_0 = tensor[0].unsqueeze(2).unsqueeze(3).unsqueeze(4).unsqueeze(5).unsqueeze(6)  # (batch_size, K_SIZE, 1, 1, 1, 1, 1)
        tensor_1 = tensor[1].unsqueeze(1).unsqueeze(3).unsqueeze(4).unsqueeze(5).unsqueeze(6)  # (batch_size, 1, K_SIZE, 1, 1, 1, 1)
        tensor_2 = tensor[2].unsqueeze(1).unsqueeze(2).unsqueeze(4).unsqueeze(5).unsqueeze(6)  # (batch_size, 1, 1, K_SIZE, 1, 1, 1)
        tensor_3 = tensor[3].unsqueeze(1).unsqueeze(2).unsqueeze(3).unsqueeze(5).unsqueeze(6)  # (batch_size, 1, 1, 1, K_SIZE, 1, 1)
        tensor_4 = tensor[4].unsqueeze(1).unsqueeze(2).unsqueeze(3).unsqueeze(4).unsqueeze(6)  # (batch_size, 1, 1, 1, 1, K_SIZE, 1)
        tensor_5 = tensor[5].unsqueeze(1).unsqueeze(2).unsqueeze(3).unsqueeze(4).unsqueeze(5)  # (batch_size, 1, 1, 1, 1, 1, K_SIZE)
        result = tensor_0 + tensor_1 + tensor_2 + tensor_3 + tensor_4 + tensor_5

        action_features = result.reshape(batch_size, K_SIZE**N_ROBOTS)
