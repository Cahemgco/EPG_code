...
class QNet(nn.Module):
    ...
    def output_q_values(self, enhanced_node_feature, edge_inputs, current_index, edge_padding_mask, node_padding_mask):
        ...
        result = torch.zeros((batch_size, K_SIZE, K_SIZE), device=action_features.device)

        tensor_0 = tensor[0].unsqueeze(2)
        tensor_1 = tensor[1].unsqueeze(1)
        result = tensor_0 + tensor_1
        action_features = result.reshape(batch_size, K_SIZE**N_ROBOTS)
        q_values = action_features.unsqueeze(2)
