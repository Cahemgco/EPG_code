        ...
        while True:
            ...
            if curr_episode % 1 == 0 and len(experience_buffer[0]) >= MINIMUM_BUFFER_SIZE:
                ...
                for j in range(8):
                    ...
                    tensor_0 = logps[0, :, :].unsqueeze(2)  # (batch_size, K_SIZE, 1)
                    tensor_1 = logps[1, :, :].unsqueeze(1)  # (batch_size, 1, K_SIZE)

                    result = tensor_0 + tensor_1  # (batch_size, K_SIZE, K_SIZE, K_SIZE)
                    
                    logp = result.permute(0, 2, 1).reshape(BATCH_SIZE, K_SIZE**N_ROBOTS, -1)  # (batch_size, K_SIZE**N_ROBOTS, 1)
                    ...
                    with torch.no_grad():
                        ...
                        tensor_0 = next_logps[0, :, :].unsqueeze(2)  # (batch_size, K_SIZE, 1)
                        tensor_1 = next_logps[1, :, :].unsqueeze(1)  # (batch_size, 1, K_SIZE)

                        result = tensor_0 + tensor_1  # (batch_size, K_SIZE, K_SIZE, K_SIZE)

                        next_logp = result.permute(0, 2, 1).reshape(BATCH_SIZE, K_SIZE**N_ROBOTS, -1)
